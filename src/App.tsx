...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (hash.startsWith("#data=")) {
      const compressed = decodeURIComponent(hash.substring(6))
      const binaryStr = LZString.decompressFromBase64(compressed)!
  ...
  const handleUpload = async (file: File) => {
    ...
    const compressed = LZString.compressToBase64(atob(base64))
    setGeneratedUrl(`${location.origin}${location.pathname}#data=${encodeURIComponent(compressed)}`)
  }
    ...
    <>
      <div className="container">
        ...
        <div>
          ...
          <input type="file" accept=".zip" className="fileInput" onChange={e => e.target.files?.[0] && handleUpload(e.target.files[0])} />
        </div>
        {generatedUrl && (
          <div>
      ...
      <footer>
        <div className="social-icons">
          ...
          <a href="https://discord.com/invite/3n3EHtyQf7" target="_blank" aria-label="Discord">
            <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="currentColor" className="bi bi-discord" viewBox="0 0 16 16">
